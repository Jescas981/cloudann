...
void LabelDefinition::addLabel(uint8_t id, const std::string& name, const Eigen::Vector3f& color)
{
    if (hasLabel(id)) {
        CC_CORE_WARN("Label with ID {} already exists, updating instead", id);
    ...
    CC_CORE_INFO("Added label: {} - {}", id, name);
}
...
void LabelDefinition::removeLabel(uint8_t id)
{
    ...
    if (it == labelIndexMap_.end()) {
        CC_CORE_WARN("Cannot remove label {}: not found", id);
        return;
    }
    ...
    CC_CORE_INFO("Removed label ID {}", id);
}

void LabelDefinition::updateLabel(uint8_t id, const std::string& name, const Eigen::Vector3f& color)
{
    auto* label = getLabel(id);
    if (!label) {
        CC_CORE_WARN("Cannot update label {}: not found", id);
        return;
    }

    label->name = name;
    label->color = color;
    CC_CORE_INFO("Updated label: {} - {}", id, name);
}
...
void LabelDefinition::loadLiDARLabels()
{
    ...
    CC_CORE_INFO("Loaded LiDAR classification labels");
}
