...
public:
  ...
  // Label management
  void addLabel(uint8_t id, const std::string &name,
                const Eigen::Vector3f &color);
  void removeLabel(uint8_t id);
  void updateLabel(uint8_t id, const std::string &name,
                   const Eigen::Vector3f &color);
...
private:
  ...
  std::map<uint8_t, size_t>
      labelIndexMap_; // Maps ID to index in labels_ vector
};
